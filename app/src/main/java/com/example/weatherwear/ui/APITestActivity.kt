        ...
        call.enqueue(object : Callback<RWCResponse> {
            override fun onResponse(call: Call<RWCResponse>, response: Response<RWCResponse>) {
                if (response.isSuccessful) {
                    ...
                    displayResponse(rwcResponse)
                } else {
                    Toast.makeText(this@APITestActivity, "Error: ${response.code()}", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<RWCResponse>, t: Throwable) {
                Toast.makeText(this@APITestActivity, "Failed: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
